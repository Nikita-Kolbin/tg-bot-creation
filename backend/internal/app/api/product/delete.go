...
// @Router       /bot/{bot_id}/product/{product_id} [delete]
func (i *Product) DeleteProduct(w http.ResponseWriter, r *http.Request) {
	...
	if !isOwner {
		logger.Error(ctx, "attempt to delete product by non-owner", "bot_id", botID, "user_id", userID)
	...
	// Проверяем, что товар принадлежит боту
	...
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			logger.Info(ctx, "product not found for deletion", "product_id", productID)
			render.Status(r, http.StatusNotFound)
			render.JSON(w, r, dto.NewErrResp("product not found"))
			return
		}
		logger.Error(ctx, "failed to delete product", "err", err, "product_id", productID)
		render.Status(r, http.StatusInternalServerError)
		render.JSON(w, r, dto.NewErrResp("failed to delete product"))
		return
	}

	logger.Info(ctx, "product deleted", "product_id", productID, "bot_id", botID)
